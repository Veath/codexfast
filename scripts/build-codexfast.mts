...
} from "./build-codexfast-utils.mts";

const rootDir = resolve(fileURLToPath(new URL("..", import.meta.url)));
const sourceDir = join(rootDir, "src");
const outputPath = join(rootDir, "bin", "codexfast");
const checkOnly = process.argv.includes("--check");
const distributionCheckOnly = process.argv.includes("--check-distribution");
...
const cliModulePattern = String.raw`\.\/cli-(?:app-environment|cdp|command-policy|context|output|platform|runtime-launch|runtime-patcher|update-settings|utils|watcher)\.mts`;
...
const cliSource = inlineSupportedAppVersions(
  insertAfterImports(
...
`;
const generated = finalizeDistributionEntrypoint(`#!/usr/bin/env node
// This file is generated by scripts/build-codexfast.mts. Do not edit it directly.
"use strict";
${nodeVersionGuard}${transpiledCliSource}`);

if (checkOnly || distributionCheckOnly) {
  const current = readFileSync(outputPath, "utf8");
  if (distributionCheckOnly) {
    try {
      assertDistributionEntrypoint(current);
    } catch (error) {
      console.error(error instanceof Error ? error.message : String(error));
      process.exit(1);
    }
  }
  const matches = distributionCheckOnly
    ? current === generated
