...
使用任意 ChatGPT 账户（包括免费账户）登录 Codex CLI 或 Codex.app，并将以下内容写入 `~/.codex/config.toml`：
...
最新完成本地验证的版本：`ChatGPT.app` / `Codex.app` `26.825.51511`（`build 7377`）。该版本改名后的 renderer 和 `.vite/build` chunk 继续兼容 Fast、`/fast`、settings schema 和按源码签名发现 updater 的路径；当前 General Settings 行形状仍由泛化的 automatic-update target 覆盖。GPT-5.6 和 Plugins 继续使用官方路径。

另已验证支持 `ChatGPT.app` / `Codex.app` `26.825.41651`（`build 7345`），其改名后的 renderer 和 `.vite/build` chunk 保持相同兼容性。

另已验证支持 `ChatGPT.app` / `Codex.app` `26.820.60940`（`build 7119`），其改名后的 renderer 和 `.vite/build` chunk 保持相同兼容性。
...
另已验证支持 `ChatGPT.app` / `Codex.app` `26.818.31338`（`build 6892`）；该版本改名后的 General Settings 模块仍可通过专用 runtime patch 恢复 renderer 的 `Disable automatic updates` 设置行，同时保留 runtime automatic-update hook 和 settings schema。

另已验证支持 `ChatGPT.app` / `Codex.app` `26.814.41407`（`build 6720`）、`26.810.52044`（`build 6662`）、`26.810.41047`（`build 6570`）、`26.803.61601`（`build 6396`）、`26.803.41515`（`build 6321`）、`26.730.61639`（`build 6234`）和 `26.730.61309`（`build 6223`）。
