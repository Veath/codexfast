...
- For `26.810.52044+6662`, confirm the official Plugins/GPT-5.6 paths remain unpatched, `app-initial-BCLYDefw.js` receives the shared Fast, three-argument request-tier, configured-tier fallback, `/fast`, and renderer schema targets, `general-settings-yzMrcfmm.js` receives Settings Fast, and `.vite/build/window-all-closed-DqXEFZQJ.js` receives the source-signature updater hook while manual update methods remain intact. Confirm that no renderer `disableAutomaticUpdates` row is present. The current app process must not be closed or relaunched for this validation unless the user explicitly requests it.
- For `26.814.41407+6720`, confirm the extracted `app.asar` matches build `6662` byte-for-byte, then apply the same target and absence checks. The current app process must not be closed or relaunched for this validation unless the user explicitly requests it.
- For `26.818.31338+6892`, confirm the official Plugins/GPT-5.6 paths remain unpatched, `app-initial-B2RlNf_b.js` receives the shared Fast, three-argument request-tier, configured-tier fallback, `/fast`, and renderer schema targets, `general-settings-DOzsH83_.js` receives Settings Fast plus `disable-automatic-updates-setting-26818`, and `.vite/build/window-all-closed-BazhJdtt.js` receives the source-signature updater hook while manual update methods remain intact. Confirm the injected renderer row after a fully quit-and-relaunched session. The current app process must not be closed or relaunched for this validation unless the user explicitly requests it.
- For `26.818.41509+6962`, confirm the official Plugins/GPT-5.6 paths remain unpatched, `app-initial-DwVrCWuo.js` receives the shared Fast, three-argument request-tier, configured-tier fallback, `/fast`, and renderer schema targets, `general-settings-DqMQqZ_N.js` receives Settings Fast plus `disable-automatic-updates-setting-26818`, and `.vite/build/window-all-closed-CSZ7EgG_.js` receives the source-signature updater hook while manual update methods remain intact. Confirm the injected renderer row after a fully quit-and-relaunched session. The current app process must not be closed or relaunched for this validation unless the user explicitly requests it.
- For `26.818.61809+7019`, confirm the official Plugins/GPT-5.6 paths remain unpatched, `app-initial-q5My48Y-.js` receives the shared Fast, three-argument request-tier, configured-tier fallback, `/fast`, and renderer schema targets, `general-settings-D4Zn65Pe.js` receives Settings Fast plus `disable-automatic-updates-setting-26818`, and `.vite/build/window-all-closed-BgGkNcoi.js` receives the source-signature updater hook while manual update methods remain intact.
- For `26.820.60940+7119`, confirm the official Plugins/GPT-5.6 paths remain unpatched, `app-initial-CmWKLN1D.js` receives the shared Fast, three-argument request-tier, configured-tier fallback, `/fast`, and renderer schema targets, `general-settings-C6MQqx12.js` receives Settings Fast plus the generalized `disable-automatic-updates-setting-26818` row replacement, and `.vite/build/window-all-closed-CZBXnSMj.js` receives the source-signature updater hook while manual update methods remain intact. Confirm the injected renderer row after a fully quit-and-relaunched session; extracted-bundle validation alone does not satisfy that UI check.
...
- `26.818.41509` also skips the legacy `Plugins access` initial target because Plugins uses the official app path.
- `26.818.61809` and `26.820.60940` also skip the legacy `Plugins access` initial target because Plugins uses the official app path.
