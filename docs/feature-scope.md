...
- On `26.810.52044` and `26.814.41407`, the packaged renderer no longer contains this switch, so no Settings row is injected. On `26.818.31338`, the dedicated General Settings target injects the switch while the main-process schema and runtime hook remain supported.
