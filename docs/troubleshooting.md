...
- `26.810.52044` also skips the legacy `Plugins access` initial target because Plugins uses the official app path.
- `26.814.41407` also skips the legacy `Plugins access` initial target because Plugins uses the official app path.
- `26.818.31338` also skips the legacy `Plugins access` initial target because Plugins uses the official app path.
- `26.818.41509`, `26.818.61809`, and `26.820.60940` also skip the legacy `Plugins access` initial target because Plugins uses the official app path.
...
- On build `26.715.61943+5628`, the same callback-aware source-signature hook applies in `.vite/build/window-all-closed-CZr9g6FK.js` after another renderer asset rename.
...
- On build `26.721.41059+5848`, the same callback-aware source-signature hook applies in `.vite/build/window-all-closed-a9FCmS92.js`; renderer Fast, `/fast`, and Intelligence Speed targets remain consolidated in `app-initial-BHB6SClA.js`, while Settings Fast and the app-name-aware automatic-update row moved to `general-settings-D7HslvR1.js` without requiring new signatures.
- On build `26.721.81911+5973`, the same callback-aware source-signature hook applies in `.vite/build/window-all-closed-a9FCmS92.js`; renderer Fast, `/fast`, and Intelligence Speed targets remain consolidated in `app-initial-CRKqnyc3.js`, while Settings Fast and the app-name-aware automatic-update row moved to `general-settings-CcPMYysK.js` without requiring new signatures.
- On build `26.727.40816+6067`, the same callback-aware source-signature hook applies in `.vite/build/window-all-closed-Coc41Tfs.js`; renderer Fast, `/fast`, Intelligence Speed, and the renderer settings schema remain consolidated in `app-initial-DRyZ1Lin.js`, while Settings Fast and the app-name-aware automatic-update row moved to `general-settings-BtLJps-O.js` without requiring new signatures. Main-process schema copies remain discoverable in `child-process-snapshot-worker.js`, `src-CLstCQVF.js`, and `worker.js`.
- On build `26.727.51351+6119`, the same callback-aware source-signature hook applies in `.vite/build/window-all-closed-Coc41Tfs.js`; renderer Fast, `/fast`, Intelligence Speed, and the renderer settings schema remain consolidated in `app-initial-iBPGfcXU.js`, while Settings Fast and the app-name-aware automatic-update row moved to `general-settings-BBCiVbba.js` without requiring new signatures. Main-process schema copies remain discoverable in `child-process-snapshot-worker.js`, `src-CLstCQVF.js`, and `worker.js`.
- On build `26.730.61309+6223`, the same callback-aware source-signature hook applies in `.vite/build/window-all-closed-DJDXIcEI.js`; renderer Fast, `/fast`, Intelligence Speed, and the renderer settings schema remain consolidated in `app-initial-YjNFxVhk.js`, while Settings Fast and the app-name-aware automatic-update row moved to `general-settings-DzOMKOoh.js` without requiring new signatures. Main-process schema copies remain discoverable in `child-process-snapshot-worker.js`, `src-Bn_6ASpg.js`, and `worker.js`.
- On build `26.730.61639+6234`, the same callback-aware source-signature hook applies in `.vite/build/window-all-closed-DJDXIcEI.js`; renderer Fast, `/fast`, Intelligence Speed, and the renderer settings schema remain consolidated in `app-initial-CKNQDTeE.js`, while Settings Fast and the app-name-aware automatic-update row moved to `general-settings-2iEePJwo.js` without requiring new signatures. Main-process schema copies remain discoverable in `child-process-snapshot-worker.js`, `src-Bn_6ASpg.js`, and `worker.js`.
- On build `26.803.41515+6321`, the updater moved to `.vite/build/window-all-closed-9IR0zY5D.js` and now calls background checks through a nested `p` callback plus `setInterval(()=>void p(),e)`. The source-signature hook must wrap that callback dynamically; renderer Fast, `/fast`, Intelligence Speed, and the renderer settings schema remain in `app-initial-Biw83Aiz.js`, while Settings Fast and the app-name-aware automatic-update row are in `general-settings-BseQIe_j.js`. Main-process schema copies remain discoverable in `child-process-snapshot-worker.js`, `src-Cz_uUmVl.js`, and `worker.js`.
- On build `26.803.61601+6396`, the same nested callback updater remains in `.vite/build/window-all-closed-9IR0zY5D.js`; renderer Fast, `/fast`, Intelligence Speed, and the renderer settings schema moved to `app-initial-BYOVlUBL.js`, while Settings Fast and the app-name-aware automatic-update row moved to `general-settings-Dz0zP8tf.js`. Main-process schema copies remain discoverable in `child-process-snapshot-worker.js`, `src-Cz_uUmVl.js`, and `worker.js` without new signatures.
- On build `26.810.41047+6570`, the updater source-signature hook matches `.vite/build/window-all-closed-B4DXN-uV.js`; renderer Fast, service-tier, `/fast`, and the renderer settings schema are in `app-initial-iMhn6nFd.js`, while Settings Fast and the app-name-aware automatic-update row are in `general-settings-mYHA2JS4.js`. Main-process schema copies remain discoverable in `child-process-snapshot-worker.js`, `src-BlUt09P1.js`, and `worker.js`. The Intelligence Speed component no longer has the historical availability gate, so a missing menu should be diagnosed through service-tier option data rather than a new UI regex.
- On build `26.810.52044+6662`, the updater source-signature hook matches `.vite/build/window-all-closed-DqXEFZQJ.js`; renderer Fast, service-tier, the three-argument request helper, `/fast`, and the renderer settings schema are in `app-initial-BCLYDefw.js`, while Settings Fast is in `general-settings-yzMrcfmm.js`. Main-process schema copies remain discoverable in `child-process-snapshot-worker.js`, `src-DY9Aq019.js`, and `worker.js`. This build has no renderer `disableAutomaticUpdates` row, so a missing Settings switch is expected; the runtime updater hook still preserves manual update actions.
- Build `26.814.41407+6720` ships the same extracted `app.asar` contents as `26.810.52044+6662`, so the same target files, updater behavior, and missing renderer Settings switch apply.
- On build `26.818.31338+6892`, the updater source-signature hook matches `.vite/build/window-all-closed-BazhJdtt.js`; renderer Fast, service-tier, the three-argument request helper, `/fast`, and the renderer settings schema are in `app-initial-B2RlNf_b.js`, while Settings Fast and the `disable-automatic-updates-setting-26818` target are in `general-settings-DOzsH83_.js`. Main-process schema copies remain discoverable in `src-Bqg9CB1K.js` and `worker.js`. If the switch is missing after a fresh relaunch, verify that the launch output reports `Disable automatic updates setting` and that the runtime interceptor stayed attached for the lazy Settings chunk; the runtime updater hook still preserves manual update actions.
- On build `26.818.41509+6962`, the same target family moved to `app-initial-DwVrCWuo.js`, `general-settings-DqMQqZ_N.js`, `src-CLzQUgbV.js`, `worker.js`, and `.vite/build/window-all-closed-CSZ7EgG_.js`.
- On build `26.818.61809+7019`, the same target family moved to `app-initial-q5My48Y-.js`, `general-settings-D4Zn65Pe.js`, `src-DlBR1tzg.js`, `worker.js`, and `.vite/build/window-all-closed-BgGkNcoi.js`.
- On build `26.820.60940+7119`, the targets are in `app-initial-CmWKLN1D.js`, `general-settings-C6MQqx12.js`, `src-DpTP2OT9.js`, `worker.js`, and `.vite/build/window-all-closed-CZBXnSMj.js`. If patching reports `Unable to capture row component`, confirm the generated CLI includes the generalized `disable-automatic-updates-setting-26818` row-local capture before retrying a fully quit launch.
...
- On `26.721.31836` (`build 5828`), `26.721.41059` (`build 5848`), `26.721.81911` (`build 5973`), `26.727.40816` (`build 6067`), and `26.727.51351` (`build 6119`), codexfast also skips Plugins runtime targets because Plugins is supported by the official app path for this repo's target use case.
- On `26.730.61309` (`build 6223`), codexfast also skips Plugins runtime targets because Plugins is supported by the official app path for this repo's target use case.
- On `26.730.61639` (`build 6234`), codexfast also skips Plugins runtime targets because Plugins is supported by the official app path for this repo's target use case.
...
Builds `26.715.21425+5488`, `26.715.31925+5551`, `26.715.52143+5591`, `26.715.61943+5628`, `26.715.70719+5650`, `26.715.72028+5706`, `26.715.72359+5718`, `26.721.30844+5813`, `26.721.31836+5828`, `26.721.41059+5848`, `26.721.81911+5973`, `26.727.40816+6067`, and `26.727.51351+6119` use the official Plugins path and do not require the legacy `Plugins access` initial target or Plugins runtime target family.

Build `26.730.61309+6223` also uses the official Plugins path and does not require the legacy `Plugins access` initial target or Plugins runtime target family.

Build `26.730.61639+6234` also uses the official Plugins path and does not require the legacy `Plugins access` initial target or Plugins runtime target family.
