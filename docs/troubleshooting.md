...
- On build `26.818.31338+6892`, the updater source-signature hook matches `.vite/build/window-all-closed-BazhJdtt.js`; renderer Fast, service-tier, the three-argument request helper, `/fast`, and the renderer settings schema are in `app-initial-B2RlNf_b.js`, while Settings Fast is in `general-settings-DOzsH83_.js`. Main-process schema copies remain discoverable in `src-Bqg9CB1K.js` and `worker.js`. This build has no renderer `disableAutomaticUpdates` row, so a missing Settings switch is expected; the runtime updater hook still preserves manual update actions.
